  ...
  void initState() {
    ...
    Future.microtask(() {
      ...
      if (context.read<SearchProductProvider>().productFilter == null) {
        products.getProductFilters(context,widget.searchkey!=""?widget.searchkey:products.searchKey);
      } else {
        context.read<SearchProductProvider>().refresh();
      }
      products.initProductList();
      products.setSearchKey(widget.searchkey!=""?widget.searchkey:products.searchKey); //save search key
      products.getProductList(
          pageNo.value,
          products.categoryIDs.isEmpty
              ? widget.categoryID!
              : products.categoryIDs,products.searchKey,
          widget.filter,
          widget.sort,);
    ...
    return Container(
      ...
      child: SafeArea(
        ...
        child: Scaffold(
            ...
            body: Consumer<SearchProductProvider>(builder: ((context, value, child) {
    ...
    if ((value.items!.length) < totalLength!) {
      ...
      await context.read<SearchProductProvider>().getProductList(
          ...
          context.read<SearchProductProvider>().categoryIDs.isEmpty
              ? widget.categoryID!
              : context.read<SearchProductProvider>().categoryIDs,value.searchKey,
          widget.filter,
          widget.sort,);
                    ...
                    : Flexible(
                        ...
                        child: InkWell(
                          onTap: () async {
                            log("filter");
                            Navigator.pushNamed(
                                context, RouteGenerator.routeSearchProductFilter);
